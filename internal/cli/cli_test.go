...
func TestStatusShowsValidationGateAndReason(t *testing.T) {
	...
	defer db.Close()
	snapshot := model.NewSnapshot("validation-project")
	snapshot.Tasks["task"] = &model.Task{ID: "task", Title: "focused", State: model.Review, Evidence: &model.Evidence{ValidationGate: "focused", ValidationReason: "low-risk change is confined to ./internal/demo", ValidationInput: strings.Repeat("a", 64)}}
	if err = db.Save(strings.Repeat("b", 40), snapshot); err != nil {
		t.Fatal(err)
	}
	var out bytes.Buffer
	cmd := New()
	...
	for _, want := range []string{"Validation: focused", "low-risk change is confined", "aaaaaaaaaaaa"} {
		if !strings.Contains(out.String(), want) {
			t.Fatalf("status omitted %q: %s", want, out.String())
		}
	}
