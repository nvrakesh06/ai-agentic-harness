...
func Compile(e config.Effective, r Role, osName string, t *model.Task, objective, diff, evidence string) string {
	...
	if r.Stage == "review" {
		if r.Name == "qa" {
			b.WriteString("\nREVIEW COORDINATION\nPeer reviews run concurrently and independently before this QA wave when reviewer, security, or designer roles are required. The evidence payload intentionally contains no peer approvals, only their completed exact-head summaries and dispositions. Inspect that evidence as input, but perform an independent acceptance assessment and do not treat a peer summary as QA approval. Do not wait for, require, or infer another reviewer's result beyond the supplied completed evidence. Treat exact-head native evidence supplied by the supervisor as authoritative. If supervisor-owned evidence is missing or must be refreshed, request that evidence without asking a human to run tools. If browser launch is denied, explicitly request a native screenshot or browser capture; do not claim a visual pass from build or HTTP checks. A restricted worker's missing Node, npm, Bun, or raw native logs is not a finding when supervisor evidence is supplied. Use blocked only for a consequential human decision the supervisor cannot make. Review the requested change, its acceptance criteria, direct callers and consumers, tests, data contracts, and adjacent security boundaries; do not hunt unrelated code. Report concrete defects in that causal surface as structured findings with severity, location, reason, suggested resolution, and relevance: changed, causal, baseline, or unknown. A location outside the diff can be causal and must remain blocking. If an incidental issue is proven to reproduce at the exact base revision, record it once as baseline with baseline_sha and concise baseline_evidence; do not expand the origin review around it. Uncertain relevance, any security concern, and high or critical findings remain blocking.\n")
		} else {
			b.WriteString("\nREVIEW COORDINATION\nPeer reviews in this wave run concurrently and independently. The evidence payload intentionally contains no peer approvals. Do not wait for, require, or infer another reviewer's result. Treat exact-head native evidence supplied by the supervisor as authoritative. If supervisor-owned evidence is missing or must be refreshed, request that evidence without asking a human to run tools. If browser launch is denied, explicitly request a native screenshot or browser capture; do not claim a visual pass from build or HTTP checks. A restricted worker's missing Node, npm, Bun, or raw native logs is not a finding when supervisor evidence is supplied. Use blocked only for a consequential human decision the supervisor cannot make. Review the requested change, its acceptance criteria, direct callers and consumers, tests, data contracts, and adjacent security boundaries; do not hunt unrelated code. Report concrete defects in that causal surface as structured findings with severity, location, reason, suggested resolution, and relevance: changed, causal, baseline, or unknown. A location outside the diff can be causal and must remain blocking. If an incidental issue is proven to reproduce at the exact base revision, record it once as baseline with baseline_sha and concise baseline_evidence; do not expand the origin review around it. Uncertain relevance, any security concern, and high or critical findings remain blocking.\n")
		}
	}
